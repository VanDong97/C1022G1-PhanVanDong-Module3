...
insert into instructor(`name`,birthday, salary)
 ...
 select student.name,room.name from student join room on student.class_id = room.id;
 
 select student.name,room.name,class.name from student left join room on student.class_id = room.id left join class on student.class_id = class.id;
 ...
 select point,count(*) from student group by point;
 
 select point,count(*) from student where point > 5 group by point;
 
 select point,count(*) from student where point > 5 group by point having count(*) >= 2;
 
 select student.name from student join room on student.class_id = room.id where room.name = 'c1121g1' order by student.name;
